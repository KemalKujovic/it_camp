...
if(session_id !== ""){
    ...
    async function populateUserDataa(){
    let user = new User();
...
document.querySelector('#deleteProfile').addEventListener('click', e => {
    ...
    let text = 'Da li ste sigurni da zelite da obrisite profile';
    ...
    async function getPostUser(){
        ...
        if(comments.length > 0){
            comments.forEach(comment => {
                comments_html += `<div class="single-comment"><p><b>Author: ${current_user.username}</p><hr>${comment.content}</div>`;
            })
        }
...
getAllPosts();
 ...
 const commentPostSubmit = e => {
    e.preventDefault();
    async function getAllCommentss(){
    let current_user = new User();
    current_user = await current_user.get(session_id);
    ...
    main_post_el.querySelector('.post-comments').innerHTML +=`<div class="single-comment"><p><b>Author: ${current_user.username}</p><hr>${comment_value}</div>`;
